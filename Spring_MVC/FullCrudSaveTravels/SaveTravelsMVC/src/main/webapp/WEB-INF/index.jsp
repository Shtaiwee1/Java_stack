			...
			<div class = "col-5 align-self-center ">
				...
				<table class="table border col-5 mb-5">
				  ...
				  <c:forEach var="item" items="${expenses}">
				    <tr>
				      <th scope="row"><a href="/expenses/${item.id}"><c:out value="${item.name}"></c:out></a></th>
				      <td><c:out value="${item.vendor}"></c:out></td>
				      <td><c:out value="${item.amount}"></c:out>$</td>
				      <td style="display:flex;justify-content: space-between;border:none;background:none"><a href="/expenses/edit/${item.id}">edit</a><form action="/expenses/delete/${item.id}" method="post">
